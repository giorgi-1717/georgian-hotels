...
async function register() {
    ...
    if (!name || !email || !phone || !password || !confirm) { alert("შეავსე ყველა ველი"); return; }
    if (!validateEmail(email)) { alert("არასწორი ელ-ფოსტა"); return; }
    if (password.length < 6) { alert("პაროლი უნდა იყოს მინიმუმ 6 სიმბოლო"); return; }
    if (password !== confirm) { alert("პაროლები არ ემთხვევა"); return; }
...
async function login() {
    ...
    if (!email || !password) { alert("შეავსე ყველა ველი"); return; }
